---
title: "技術情報収集を自動化したら、情報収集疲れがなくなった（n8n）"
emoji: "📰"
type: "tech"
...
今更ながら [n8n](https://n8n.io/) の環境を構築してみて、手始めに技術系の情報収集を自動化するシステムを作ったので、その備忘録を残します。
...
フロントエンドに関するトピック（6数時間ごとに Discord に届く）↓
...
- **Add Feed Info**：サブワークフローにフィードソース情報を渡しています
  - n8n の Split In Batches（ループ処理）はネストできない制約があるようです（実際にやってみたらループ結果がおかしくなりました）
...
記事を 1 件ずつ処理し、AI で分析して DB に保存するサブワークフローです。
...
- **テストモード**：サブワークフロー化したことで、単体テスト実行時にデータを渡せないため追加
